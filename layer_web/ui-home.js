...
function weatherLine(){const sky=state.sky||"clear";const t=state.tempC;const map={thunder:"Thunderstorm. Keep it close to skin.",rain:"Rain. Fresh notes will cut through the wet air.",snow:"Snow. Reach for something rich and warm.",fog:"Fog. Musks and woods will hold.",haze:"Hazy. Something clean, not heavy.",wind:"Windy. A bit more sillage will survive.",cloudy:"Overcast. Balanced layers work.",partly:"Partly cloudy · "+t+"°",clear:state.weather==="hot"?"Clear and sunny. Try something fresh.":"Clear · "+t+"°"};return (map[sky]||map.clear)}
...
function journeyCard(p,when){const pct=matchPct(p);const src=bottleSrc(p);return `<button class="journey" onclick="openDetail('${p.id}')"><div class="journey-thumb"><img alt="${p.name}" src="${src}" onerror="this.src='${PHOTOS[0]}'"></div><div class="journey-body"><h4>${p.name}</h4><div class="journey-meta">${when} · ${state.sky||state.weather} · ${notesShort(p)}</div><div class="bar"><i style="width:${pct}%"></i></div><div class="match">${pct}% match</div></div></button>`}
